...
public class Vassal : CardType
{
    ...
    {
        MakeDecision.inst.ChooseDisplayOnScreen(canHurt, AutoTranslate.Target_Instruction(player.name, thisCard.name), HurtCard, true);
    ...
    void PartTwo(Player player, Card thisCard, int logged)
    {
        ...
        MakeDecision.inst.ChooseDisplayOnScreen(availableTroops, AutoTranslate.Target_Instruction(player.name, thisCard.name), ProtectCard, true);
