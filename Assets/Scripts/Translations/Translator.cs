...
/*
    public string SplitAndTranslate(string logText, int owner = -1, int indent = 0)
    {
        string targetText = "";
        ...
        string main = splitUp[0].Replace(" ", "_");
        ...
        if (TranslationExists($"{main}_Others") && (int)PhotonNetwork.LocalPlayer.CustomProperties[ConstantStrings.MyPosition] != owner)
            targetText += Translate($"{main}_Others", toTranslate);
        else
