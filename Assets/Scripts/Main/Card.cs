...
#region Properties
    ...
    public void StunRPC(Player player, int increment, int logged = 0)
    {
        ...
        Log.inst.AddMyText(true, OnlineTranslate.Online_Stun_Card(this.name, roundNumber.ToString()), logged);
    ...
    {
        int roundNumber = (int)GetRoomProperty(ConstantStrings.CurrentRound) + increment;
        Log.inst.AddMyText(true, OnlineTranslate.Online_Protect_Card(this.name, roundNumber.ToString()), logged);
    ...
    public void HealthRPC(Player player, int num, int logged = 0)
    {
        if (num == 0)
            ...
            Log.inst.AddMyText(false, OnlineTranslate.Online_Add_Health_Card(player.name, this.name, num.ToString()), logged);
        else if (num < 0 && CanTakeDamage())
            Log.inst.AddMyText(false, OnlineTranslate.Online_Lose_Health_Card(player.name, this.name, Mathf.Abs(num).ToString()), logged);
        else
            return;
